...
func certIsRevoked(cert *x509.Certificate, crl *pkix.CertificateList) bool {
	if crl.HasExpired(time.Now()) {
		logger.Log.Warnf("The current Certificate Revocation List (CRL) is past expiry date and must be updated. Revoked certificates will still be rejected in this state.")
	}
